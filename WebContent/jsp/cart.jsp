...
<html>
	...
	<script type="text/javascript">
		 //修改订单项数量，刷新整个页面
		/* function updateCartItem(pid,id) {
			var count = $("#"+id).val() ;
			window.location.href = "${ pageContext.request.contextPath }/CartServlet?method=upCartItem&pid=" + pid + "&count=" + count ;
		}  */
		//修改订单项数量，异步刷新部分页面
		function updateCartItem(pid,id) {
			var count = $("#"+id).val() ;
			//alert(count) ;
			$.get("${ pageContext.request.contextPath }/CartServlet",{"method":"upCartItem","pid":pid,"count":count},function(data){
				 $(data).each(function(i,n){
					 //alert(n.count) ;
					 //alert(n.total) ;
					 $("#"+id).val(n.count) ;
					 $("#"+id+"sub").text(n.subTotal) ;
					 $("#total").text(n.total) ;
					 $("#point").text(n.total/10) ;
					 	
				 }) ;
			},"json") ;
		} 
		function del(pid) {
			var flag = confirm("确定残忍的删除我吗？")
			if(flag == true) {
	...
	<%@ include file="menu.jsp" %>	
		...
		<c:if test="${ fn:length(cart.map) == 0 }">
			<center><h3>购物车空了，去购物吧，少年</h3></center>
		</c:if>
		<c:if test="${ fn:length(cart.map) != 0 }">
		...
		<div class="container">
						...
						<tbody>
							...
							<c:forEach var="entry" items="${ cart.map }" varStatus="vs">
								...
								<tr class="active">
									...
									<td width="10%">
										<input type="text" name="count" id="${ vs.count }" value="${ entry.value.count }" onchange="updateCartItem('${ entry.value.product.pid }','${ vs.count }')">
									</td>
									<td width="15%">
										<span class="subtotal" id="${ vs.count }sub">￥${ entry.value.subtotal }</span>
									</td>
									<td>
			...
			<div style="margin-right:130px;">
				...
				登录后确认是否享有优惠&nbsp;&nbsp;
			</em> 赠送积分: <em id="point" style="color:#ff6600;">${ cart.total / 10 }</em>&nbsp; 商品金额: <strong id="total" style="color:#ff6600;">￥${ cart.total }元</strong>
				</div>
